...
import {
  ArrowRight, Check, MessageCircle, ChevronRight, X,
} from "lucide-react";
...
interface RegisterFormProps {
  /**
   * Contenido opcional a mostrar junto al formulario (columna izquierda)
   * mientras el paso activo es "form". Si se omite, el formulario se
   * muestra solo, centrado — pensado para landings que ya traen su propio
   * hero/beneficios por fuera de este componente.
   */
  sidebar?: React.ReactNode;
}

/**
 * Formulario de autorregistro de dojos — POST /api/public/register.
 * Único punto de verdad: se usa tal cual en /register y en cualquier
 * landing de campaña (ej. /registro-instagram) para no duplicar la
 * lógica de validación/envío ni el comportamiento del flujo de éxito.
 */
export default function RegisterForm({ sidebar }: RegisterFormProps) {
  ...
  if (!sidebar) return <div className="max-w-lg mx-auto w-full">{formCard}</div>;
