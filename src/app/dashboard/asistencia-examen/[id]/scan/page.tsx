...
export default function ExamAttendanceScanPage() {
  ...
  useEffect(() => {
    fetch(`/api/exam-applications/${id}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (!d) return;
        setExamTitle(d.title ?? "");
        const accepted = (d.invitees ?? []).filter((i: { response: string }) => i.response === "ACCEPTED");
        setTotalCount(accepted.length);
        setArrivedCount(accepted.filter((i: { attended: boolean }) => i.attended).length);
      });
  }, [id]);

  const runCheckin = useCallback(async (val: string) => {
    const body = /^\d+$/.test(val) ? { studentCode: parseInt(val, 10) } : { studentId: val };
    const res  = await fetch(`/api/exam-applications/${id}/checkin`, {
      method:  "POST",
    ...
    if (!res.ok) {
      if (data.notEnrolled) {
        ...
        setLastResult({ type: "error", studentName: "", message: data.error ?? "QR no válido para esta postulación" });
      }
  ...
  useEffect(() => {
    ...
    import("html5-qrcode").then(({ Html5Qrcode }) => {
      if (cancelled) return;
      const qr = new Html5Qrcode("ex-qr-reader");
  ...
  const resultTitle: Record<ScanResult["type"], string> = {
    ...
    already:        "YA FUE REGISTRADO",
    "not-enrolled": "No está en esta lista",
    error:          "QR no válido",
  };
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#080C14" }}>
      <header className="h-14 flex items-center justify-between px-4 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-2.5">
          <a href={`/dashboard/asistencia-examen/${id}`}
          ...
          {view === "ready" ? (
              ...
              {cameraError ? (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-white/50 text-sm leading-relaxed max-w-xs mx-auto">
                    Apunta la cámara al código QR del alumno para registrar su llegada al examen.
                  </p>
                </div>
              )}
            ...
            <div className="space-y-4">
              <div className="relative rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: "1/1" }}>
                <div id="ex-qr-reader" className="w-full h-full" />
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="relative" style={{ width: "65%", height: "65%" }}>
                    {[
                      ...
                      <div key={cls} className={`absolute w-8 h-8 ${cls}`} style={{ borderColor: "#C0392B" }} />
                    ))}
                  </div>
                </div>
              </div>
