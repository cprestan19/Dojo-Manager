...
export default function PostulacionDetallePage() {
  ...
  async function changeStatus(action: "publish" | "close" | "finalize" | "reopen") {
    setActioning(action);
    try {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          {app.status === "FINALIZED" && (
            <button
              onClick={() => { if (confirm("¿Reabrir esta postulación? Vuelve a estado \"Cerrada\" — no se podrán agregar alumnos nuevos, solo corregir asistencia/aprobado de los que ya estaban y volver a Finalizar.")) changeStatus("reopen"); }}
              disabled={!!actioning}
              className="btn-secondary text-sm"
            >
              {actioning === "reopen" ? <Loader2 size={14} className="animate-spin" /> : "Reabrir"}
            </button>
          )}
          {app.status !== "FINALIZED" && (
