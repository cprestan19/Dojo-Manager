...
// POST /api/exam-applications/[id]/close — PUBLISHED → CLOSED
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (existing.status !== "PUBLISHED") {
      return NextResponse.json({ error: "Solo se puede cerrar desde estado PUBLISHED" }, { status: 400 });
    }
    ...
    await logAudit({
      ...ctx,
      action:       "EXAM_APPLICATION_CLOSED",
    ...
    console.error("POST /api/exam-applications/[id]/close", err);
